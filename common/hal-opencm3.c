...
#error Unsupported platform
  ...
  gpio_mode_setup(GPIOA, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE, GPIO12); // Configures GPIO12 as an output pin, necessary for capturing traces
  usart_set_baudrate(SERIAL_USART, SERIAL_BAUD);
  usart_set_databits(SERIAL_USART, 8);
  usart_set_stopbits(SERIAL_USART, USART_STOPBITS_1);
  usart_set_mode(SERIAL_USART, USART_MODE_TX_RX);
...
void send_USART_bytes(const unsigned char* in, int n) 
{
  int i; 
  for(i = 0; i < n; i++) { 
    usart_send_blocking(SERIAL_USART, in[i]); 
  } 
} 
...
{ 
  int i;
  for(i = 0; i < n; i++) { 
    out[i] = usart_recv_blocking(SERIAL_USART); 
  } 
}
