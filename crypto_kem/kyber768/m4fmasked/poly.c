...
void poly_packcompress(unsigned char *r, poly *a, int i) {
  ...
  for (j = 0; j < KYBER_N / 8; j++) {
    for (k = 0; k < 8; k++)
      t[k] =
          ((((uint32_t)a->coeffs[8 * j + k] << 11) + KYBER_Q / 2) / KYBER_Q) &
          0x7ff;
  ...
  for (j = 0; j < KYBER_N / 4; j++) {
    for (k = 0; k < 4; k++)
      t[k] =
          ((((uint32_t)a->coeffs[4 * j + k] << 10) + KYBER_Q / 2) / KYBER_Q) &
          0x3ff;
 ...
 **************************************************/
void poly_frommont(poly *r) { asm_frommont(r->coeffs); }
...
void poly_reduce_i16(int16_t *r) { asm_barrett_reduce(r); }
